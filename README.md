...
Interactive tools work on both eras: when an ephemeral server needs input mid-call (elicitation on legacy servers, `input_required` retries on 2026-07-28 servers), MCPorter identifies the requesting server and safely prompts on the terminal — or declines cleanly with a hint when running non-interactively. Programmatic `elicitationHandler` callbacks receive `(request, { server })`, allowing per-server trust policies. Daemon-managed keep-alive calls cannot forward prompts to the calling terminal yet, so they decline with the same actionable hint; set `lifecycle: "ephemeral"` when a tool must prompt interactively. `mcporter serve` bridges your configured servers to both client generations at once: one endpoint serves 2026-07-28 and 2025-era clients side by side.
...
- Keep-alive daemon calls decline server elicitation instead of prompting across the daemon socket, and print a hint in the calling CLI. Use `"lifecycle": "ephemeral"` for tools that require interactive answers.
