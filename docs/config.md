...
| `cwd`                            | Working directory for stdio servers. A leading `~` is expanded to `$HOME`; relative paths resolve against the config file directory. All paths are normalized for the host platform. Defaults to the config file directory when omitted. |
