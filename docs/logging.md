...
- For Chrome DevTools definitions, status also prints the current or last OpenClaw extension-relay decision: `relay`, `legacy`, or `unavailable`, plus its policy, classified Browser Relay Authentication v2 reason, logical upstream endpoint, and safe probe status/duration. Ordinary connection setup emits the same structured, redacted object at `MCPORTER_LOG_LEVEL=info`; it never includes relay keys, proofs, nonces, stable or ephemeral bearers, protected handoff paths, headers, pairing strings, or rewritten child arguments.
