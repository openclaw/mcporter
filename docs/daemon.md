...
Keep-alive daemons compare each applicable server's effective relay settings, using the same policy, timeout, URL validation, profile, and credential-selection resolvers as routing. Unset policy and explicit `prefer` are equivalent; policy case/whitespace and the legacy `off` alias are normalized. Timeout spellings that resolve to the same default or clamped value, default-profile aliases, and equivalent URL spellings also match. An explicit URL matching the discovered or fallback URL has the same endpoint identity. Process values fully overridden by every applicable server's `env` map do not invalidate the daemon. Values still inherited by another server, or used as raw command/relay placeholder inputs, remain identity dependencies; overrides are resolved against the process environment, without chaining server overrides.

Identity retains the effective policy, timeout, endpoint, credential directory, v2 protocol marker, derived `keyId`, and discovery inputs, including home/state/config selection and `PATH`/executable-selection variables. Discovery variables retain their values unless the shared resolver guarantees equivalence. Both daemon metadata creation and freshness checks await the same bounded asynchronous resolver; freshness never launches OpenClaw through a synchronous child process. Changing the selected OpenClaw profile or relay port, or rotating the key at the same path, therefore replaces stale daemon state on the next operation without exposing key material. Older identity metadata is replaced through the existing version check. The `require` policy remains fail-closed, and config paths/layers still isolate daemon sockets and transports: separate temporary configs, including relay-off configs, do not become a shared daemon through identity normalization.

`mcporter daemon status` shows the current or last redacted decision with route, policy, reason, safe logical upstream endpoint, and probe status/duration. Set `MCPORTER_LOG_LEVEL=info` to see the same structured decision during ordinary connection setup. Diagnostics distinguish unsupported v2 authentication, bad server proofs, unauthenticated server failures, replay, protocol, freshness, and sequence failures, disconnected extensions (an authenticated `503`), timeouts, network errors, protected handoff failures, and success. They never print keys, proofs, nonces, stable or ephemeral bearers, handoff paths, headers, pairing strings, or child arguments.
