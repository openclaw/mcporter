...
describe('generated config schema', () => {
  ...
  it('includes top-level $schema, OAuth, and tool filter properties', async () => {
    const schemaPath = new URL('../mcporter.schema.json', import.meta.url);
    ...
    expect(entryProperties?.oauthClientMetadataUrl).toBeDefined();
    expect(entryProperties?.oauth_client_metadata_url).toBeDefined();
