...
export interface TemplateInput {
  outputPath?: string;
  /**
   * Final location of the executed artifact (bundle or compiled binary) when it
   * differs from the .ts template path. Used to compute a relocation-safe
   * relative stdio cwd at generation time; defaults to outputPath.
   */
...
\t\tconst resolvedCwd = __mcpRelativeStdioCwd !== null
\t\t\t? path.resolve(__mcpScriptDir, __mcpRelativeStdioCwd)
\t\t\t: (typeof stdio.cwd === 'string' ? stdio.cwd : undefined);
\t\treturn {
\t\t\t...base,
\t\t\tcommand: {
\t\t\t\t...stdio,
 ...
 * not find its own relative args. Using process.cwd() at generation time as a
 * sensible default, and emitting it as a relocation-safe relative path, lets the
 * generated CLI resolve its spawn cwd against its own script location at
 * runtime. That way the bundle keeps working when installed globally, moved, or
...
function computeRelativeStdioCwd(definition: ServerDefinition, outputPath?: string): string | null {
  ...
  if (path.isAbsolute(relative)) {
    // Different drive on Windows; path.relative falls back to an absolute result.
    return null;
  }
